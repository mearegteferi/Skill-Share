...
from tests.utils.user import create_random_user, create_user, get_user_by_email
...
def test_create_user_new_email(
    ...
    with (
        patch("app.shared.email.send_email", return_value=None),
...
def test_retrieve_users(
    ...
    create_user(db, UserCreate(email=random_email(), password=random_lower_string()))
    create_user(db, UserCreate(email=random_email(), password=random_lower_string()))
...
def test_update_password_me(
    ...
    # Revert password to keep test suite consistent
    r = client.patch(
        f"{settings.API_V1_STR}/users/me/password",
        headers=superuser_token_headers,
        json={
...
def test_register_user(client: TestClient, db: Session) -> None:
    ...
    user_db = db.execute(select(User).where(User.email == username)).scalar_one_or_none()
    assert user_db
...
def test_update_user_email_exists(
    ...
    user1 = create_user(db, UserCreate(email=random_email(), password=random_lower_string()))
    user2 = create_user(db, UserCreate(email=random_email(), password=random_lower_string()))
    r = client.patch(
...
def test_delete_user_current_super_user_error(
    ...
    assert super_user
    r = client.delete(
        f"{settings.API_V1_STR}/users/{super_user.id}", headers=superuser_token_headers
    )
    assert r.status_code == 403
    assert r.json()["detail"] == "Super users are not allowed to delete themselves"
